...
function OAuthCallbackContent() {
  ...
  const getStatusIcon = () => {
    switch (status) {
      case 'loading': return <Loader className="w-12 h-12 text-blue-500 animate-spin" />;
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        <div className={`rounded-xl border p-8 text-center ${getStatusColor()}`}>
          ...
          <p className="text-lg mb-6">{message || 'Processing your authentication...'}</p>

          {status === 'loading' && (
            <p className="text-sm opacity-75">Please wait while we complete the authentication...</p>
          )}
          {status === 'error' && (
            <button onClick={() => router.push('/login')} className="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">
              Back to Login
            </button>
          )}
          {status === 'success' && <p className="text-sm opacity-75">Redirecting you...</p>}
        </div>

        {status === 'error' && (
          <div className="mt-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
            <h3 className="font-medium text-yellow-800 dark:text-yellow-200 mb-2">Troubleshooting Tips</h3>
