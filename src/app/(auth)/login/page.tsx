...
function LoginPageContent() {
  ...
  useEffect(() => {
    if (isAuthenticated && !authLoading && !hasRedirected) {
      ...
      router.replace(redirectParam || getDefaultRoute());
    }
  }, [isAuthenticated, authLoading, hasRedirected, isAdmin, getDefaultRoute, router, searchParams]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isAuthenticated || hasRedirected || isLoading) return;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'password') setPasswordStrength(Math.min(Math.floor(value.length / 2), 5));
    if (error) setError('');
  ...
  if (isAuthenticated && (authLoading || hasRedirected)) {
    return (
      <div className="w-full max-w-md mx-auto text-center">
        ...
        <p className="mt-4 text-gray-600">Redirecting to {isAdmin ? 'admin panel' : 'dashboard'}...</p>
        {isAdmin && <p className="mt-2 text-sm text-blue-600">Welcome, Administrator!</p>}
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-md mx-auto">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              value={formData.email} onChange={handleChange} disabled={isFormLoading}
              className="block w-full rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3 pl-10 disabled:opacity-50"
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              value={formData.password} onChange={handleChange} disabled={isFormLoading}
              className="block w-full rounded-md border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 px-3 pl-10 pr-10 disabled:opacity-50"
      ...
      <div className="mt-6">
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          <a
            ...
            onClick={(e) => { if (isFormLoading) e.preventDefault(); setIsLoading(true); }}
          >
      ...
      <p className="mt-8 text-center text-sm text-gray-600">
        ...
        <Link href="/register" className="font-medium text-blue-600 hover:text-blue-500" onClick={(e) => isFormLoading && e.preventDefault()}>
          Sign up for free
        </Link>
      </p>
